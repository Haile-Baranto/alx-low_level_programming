...
int main(int ac, char *av[])
{
	...
	mode = S_IRGRP | S_IWGRP | S_IROTH | S_IRUSR | S_IWUSR;
	if (ac != 3)
		dprintf(STDE, "Usage: cp file_from file_to\n"), exit(97);
	input_fd = open(av[1], O_RDONLY);
	if (input_fd == -1)
		dprintf(STDE, "Error: Can't read from file %s\n", av[1]), exit(98);
	output_fd = open(av[2], O_CREAT | O_WRONLY | O_TRUNC, mode);
	if (output_fd == -1)
		dprintf(STDE, "Error: Can't write to %s\n", av[2]), exit(99);
	do {
	...
	if (istatus == -1)
		dprintf(STDE, "Error: Can't close fd %d\n", input_fd), exit(100);
